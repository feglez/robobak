...
function Update-TimesLog {
    ...
    if (Test-Path $TimesLog) {
        ...
        if ($content -and $content.Trim()) {
            ...
            foreach ($block in $blocks) {
                $trimmed = $block.Trim()
                if ($trimmed -and -not $trimmed.StartsWith('#')) { $entries += , $trimmed }
            }
        }
    }
    ...
    # Build the output: header + existing entries + new entry, separated by blank lines
    $output = @("# Generated by RoboBak. Rolling log of the last 10 backups with timing information.", "")
...
#   /LOG+:file Append full output to robocopy_log.txt on the destination
#              (append mode preserves the descriptive header written before the copy)
...
# Write header to robocopy log before the copy starts (robocopy appends via /LOG+)
"# Generated by RoboBak. Full technical robocopy output for the backup operation." | Set-Content -Path $LogRobo -Encoding UTF8
...
$outputLines = @("# Generated by RoboBak. List of all backup drives with dates, sorted by name, tagged NEWEST/OLDEST.", "")
...
#   /W:5   Wait 5 seconds between retries
#   /LOG+  Append the full comparison report to verify_log.txt on the destination
#          (append mode preserves the descriptive header written before verification)
...
# Write header to verify log before verification starts (robocopy appends via /LOG+)
"# Generated by RoboBak. Robocopy verification output -- lists differences between source and destination." | Set-Content -Path $LogVerify -Encoding UTF8
